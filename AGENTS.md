...
- `stock_opname`: physical counting workflow. Completion requires every snapshotted row to have an `actual_quantity` and records `completed_by` / `completed_at` for auditability
