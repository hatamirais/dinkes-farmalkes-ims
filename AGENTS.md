...
- `items`: master data and item catalog; items may be flagged as program item `[P]` (`is_program_item`) or essential `[E]` (`is_essential`), and may belong to multiple `Terapi Obat` groups through the `TherapeuticClass` lookup for reporting
