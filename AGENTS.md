...
- `stock_opname`: physical counting workflow. Completion requires every snapshotted row to have an `actual_quantity`, records `completed_by` / `completed_at` for auditability, caps header notes at 1000 chars and row notes at 255 chars, and rate-limits POST mutations with separate stock-opname form/input/workflow buckets (`STOCK_OPNAME_FORM_RATE_LIMIT`, `STOCK_OPNAME_INPUT_RATE_LIMIT`, `STOCK_OPNAME_WORKFLOW_RATE_LIMIT`) while keeping `STOCK_OPNAME_MUTATION_RATE_LIMIT` as a compatibility fallback
...
- Current settings-backed knobs are `USER_BULK_ACTION_RATE_LIMIT`, `USER_MUTATION_RATE_LIMIT`, `ITEM_MUTATION_RATE_LIMIT`, `USER_PASSWORD_RESET_RATE_LIMIT`, `PASSWORD_CHANGE_RATE_LIMIT`, `PUSKESMAS_RECEIPT_CONFIRMATION_MUTATION_RATE_LIMIT`, `PUSKESMAS_CONSUMPTION_MUTATION_RATE_LIMIT`, `LPLPO_IMPORT_RATE_LIMIT`, `STOCK_OPNAME_FORM_RATE_LIMIT`, `STOCK_OPNAME_INPUT_RATE_LIMIT`, and `STOCK_OPNAME_WORKFLOW_RATE_LIMIT`. The legacy `PUSKESMAS_SBBK_MUTATION_RATE_LIMIT` env var remains accepted as a compatibility fallback.
