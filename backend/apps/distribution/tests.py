...
class DistributionWorkflowTest(TestCase):
    ...
    def test_submit_draft_to_submitted(self):
        ...
        response = self.client.post(reverse('distribution:distribution_submit', args=[dist.pk]))
        self.assertEqual(response.status_code, 302)
        dist.refresh_from_db()
        self.assertEqual(dist.status, Distribution.Status.SUBMITTED)

    def test_submit_requires_items(self):
        dist = self._create_distribution(status=Distribution.Status.DRAFT, with_items=False)
        response = self.client.post(reverse('distribution:distribution_submit', args=[dist.pk]))
        self.assertEqual(response.status_code, 302)
        dist.refresh_from_db()
    ...
    def test_verify_submitted_to_verified(self):
        dist = self._create_distribution(status=Distribution.Status.SUBMITTED)
        response = self.client.post(reverse('distribution:distribution_verify', args=[dist.pk]))
        self.assertEqual(response.status_code, 302)
        dist.refresh_from_db()
    ...
    def test_verify_requires_approved_qty(self):
        dist = self._create_distribution(status=Distribution.Status.SUBMITTED, with_items=False)
        DistributionItem.objects.create(
            distribution=dist,
            item=self.item,
            quantity_requested=Decimal('50'),
    ...
    def test_prepare_verified_to_prepared(self):
        ...
        response = self.client.post(reverse('distribution:distribution_prepare', args=[dist.pk]))
        self.assertEqual(response.status_code, 302)
        dist.refresh_from_db()
    ...
    def test_distribute_deducts_stock_and_creates_transaction(self):
        ...
        response = self.client.post(reverse('distribution:distribution_distribute', args=[dist.pk]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_reject_submitted_to_rejected(self):
        dist = self._create_distribution(status=Distribution.Status.SUBMITTED)
        response = self.client.post(reverse('distribution:distribution_reject', args=[dist.pk]))
        self.assertEqual(response.status_code, 302)
        dist.refresh_from_db()
    ...
    def test_edit_allowed_for_draft(self):
        dist = self._create_distribution(status=Distribution.Status.DRAFT)
        response = self.client.get(reverse('distribution:distribution_edit', args=[dist.pk]))
