...
from .services import build_stock_list_context
...
def stock_list(request):
    return render(
        ...
        build_stock_list_context(request.GET),
    )
...
def _build_stock_card_data(item, location_id=None, sumber_dana_id=None,
    ...
    # ── Group by funding source, source document, location, and batch ─
    ...
    for tx in transactions:
        ...
        if card_key not in sd_groups:
            sd_groups[card_key] = {
                ...
                "location_id": tx.location_id or 0,
    ...
    if any(
        funding_key == 0
        for funding_key, _source_key, _location_key, _batch_key in tx_price_keys
    ):
