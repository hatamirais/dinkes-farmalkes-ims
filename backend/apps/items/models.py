...
def _validate_case_insensitive_name_uniqueness(
    instance, model_class, field_name="name"
):
...
class Item(TimeStampedModel):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["kategori", "is_program_item"], name="idx_item_category_program"
            ),
        ]
    ...
    def generate_kode_barang():
        """Generate next sequential code: ITM-YYYY-00001, ITM-YYYY-00002, etc."""
        year = timezone.now().year
        prefix = f"ITM-{year}-"
        last = (
            Item.objects.filter(kode_barang__startswith=prefix)
