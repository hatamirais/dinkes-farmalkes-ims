...
RECEIVING_MUTATION_RATE_LIMIT = os.getenv("RECEIVING_MUTATION_RATE_LIMIT", "20/m")
...
PUSKESMAS_RECEIPT_CONFIRMATION_MUTATION_RATE_LIMIT = os.getenv(
    "PUSKESMAS_RECEIPT_CONFIRMATION_MUTATION_RATE_LIMIT",
    os.getenv("PUSKESMAS_SBBK_MUTATION_RATE_LIMIT", "20/m"),
)
...
# Enforced via SecurityMiddleware's SECURE_CONTENT_TYPE_NOSNIFF and a
# custom CSP header. All inline scripts have been refactored to external
# files to allow a strict script-src.
SECURE_CSP = {
    "default-src": ["'self'"],
    "script-src": ["'self'"],
    "style-src": ["'self'", "'unsafe-inline'"],  # Bootstrap relies on inline styles
    "img-src": ["'self'", "data:"],
    "font-src": ["'self'"],
    "connect-src": ["'self'"],
