...
- Kolom `expiry_date` pada `receiving.csv` kosong hanya valid untuk item dengan `requires_expiry_date=False`; untuk item lain importer menolak baris tersebut. Data lama yang memakai sentinel `2099-12-31` dimigrasikan ke `NULL` setelah field batch dibuat nullable.
