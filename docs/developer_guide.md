...
- Untuk stok awal, gunakan `receiving.csv` melalui endpoint import Receiving Admin di `/admin/receiving/receiving/import-csv/` agar stok dan `Transaction(IN)` terbentuk dalam satu alur yang konsisten. Template kosong dengan header yang diharapkan dapat diunduh dari `/admin/receiving/receiving/export-csv-template/`.
