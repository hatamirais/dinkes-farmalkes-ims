...
- Surface mobile/PWA awal tersedia di `/mobile/` untuk cek stok berbasis sesi login dan permission yang sama dengan aplikasi utama. Pengguna dengan akses stok mendapat tautan dari shell desktop dan panduan pemasangan ringan di halaman mobile.
...
- `mobile`: surface PWA server-rendered untuk penggunaan layar kecil. Versi awal menyediakan pencarian stok berbasis barang dengan total agregat, detail batch/lokasi per barang, tautan discovery dari aplikasi utama, dan panduan install PWA dengan login/session, CSRF, dan permission Django/module-scope yang sama seperti aplikasi utama.
